...
std::vector<unsigned long> read_objdump(const char *objfile, unsigned long start,
                                        unsigned long *end, bool kcore) {
    ...
    if (sscanf(raw_line, "%lx:\t%s\n", &addr, inst1)) {
      if (!strncmp(inst1, "call", 5)) {
        ...
        if (real_addr > start)
          ...
          std::cerr << "out of address space: 0x" << std::hex << real_addr
                    << std::dec << std::endl;
...
unsigned long read_kallsyms(std::string obj_sym) {
  ...
  while (getline(&raw_line, &len, kallsyms_file) != -1) {
    ...
    if (std::string(sym) == obj_sym) return addr;
  }
